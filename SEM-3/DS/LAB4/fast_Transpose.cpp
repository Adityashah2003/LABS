...
class Sparse{
    ...
    int column;
    int value;
    void create_mat(int r, int c, int v){
    ...
    void FTranspose(Sparse a[], Sparse b[]){
        ...
        b[0].create_mat(a[0].column,a[0].row,a[0].value);
        ...
        if(v>0){
            ...
            for(int i=0;i<=v;i++){
                ...
                b[j++].create_mat(a[i].column,a[i].row,a[i].value);
            }
        }
    }
    void display(){
...
int main(){
    ...
    Sparse s[count+1];
    s[0].create_mat(m,n,count);
    int k=0;
    for(int i=0;i<m;i++){
        for(int j=0;j<n;j++){
            if(arr[i][j]!=0){
                s[++k].create_mat(i,j,arr[i][j]);
            }
        }
    }
