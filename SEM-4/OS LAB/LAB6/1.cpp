...
void srtf(){
    ...
    while(comp!=n){
        ...
        if(min_idx==-1){ //no process is there yet for execution
            curr_time++;
        }
        else{
...
void rr(){
    ...
    while(comp!=n){
        ...
        if(bt_rem[p.pid]-tq>0){//if remaining burst time is more than time quantum
            ...
            curr_time += bt_rem[p.pid]; //incre curr time by bt_rem
...
int main(){
    int choice;
    cout<<"Enter your choice 1-srtf 2-rr 3-priority): ";
    cin>>choice;
